...
export async function create(data:CreateSensorInput) {
    const {bueiro_id, tipo_sensor_id, fabricante, numero_serie, data_instalacao, ultima_calibracao, status} = data
    return prisma.sensores.create({
        data:
            {bueiro_id, tipo_sensor_id, fabricante, numero_serie, data_instalacao, ultima_calibracao, status} 
    })
}
